...
function equipmentSummaryText(selection) { const m = { bodyweight: 'Bodyweight', dumbbells: 'Dumbbells', gym: 'Gym' }; const base = (selection.available || []).map((x) => m[x]).join(' + ') || 'None'; return selection.preference && selection.preference !== 'Mix' ? `${base} (${selection.preference})` : base; }
