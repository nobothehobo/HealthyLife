...
const EQUIP_PREFERENCE_OPTIONS = ['mix', 'prefer_gym', 'prefer_dumbbells', 'prefer_bodyweight'];
...
function normalizeEquipmentSelection(value) {
  ...
  const legacyMap = { 'Mix': 'mix', 'Prefer Gym': 'prefer_gym', 'Prefer Dumbbells': 'prefer_dumbbells', 'Prefer Bodyweight': 'prefer_bodyweight' };
  const prefRaw = value?.preference || 'mix';
  const preference = EQUIP_PREFERENCE_OPTIONS.includes(prefRaw) ? prefRaw : (legacyMap[prefRaw] || 'mix');
  return { available: available.length ? available : ['bodyweight', 'dumbbells'], preference };
}
async function getEquipmentSelection() { return normalizeEquipmentSelection(await getMeta('equipmentSelection', null)); }
function equipmentSummaryText(selection) { const m = { bodyweight: 'Bodyweight', dumbbells: 'Dumbbells', gym: 'Gym' }; const p = { mix: 'Mix', prefer_gym: 'Prefer Gym', prefer_dumbbells: 'Prefer Dumbbells', prefer_bodyweight: 'Prefer Bodyweight' }; const base = (selection.available || []).map((x) => m[x]).join(' + ') || 'None'; return selection.preference && selection.preference !== 'mix' ? `${base} (${p[selection.preference] || 'Mix'})` : base; }
...
function chooseItems(pool, count, favorites = [], preference = 'mix') {
  const favSet = new Set(favorites); const prefMap = { prefer_gym: 'gym', prefer_dumbbells: 'dumbbells', prefer_bodyweight: 'bodyweight' }; const pref = prefMap[preference] || null;
...
function renderWorkoutChecklist(workout) {
  ...
  $('#equipmentSummary').textContent = `Equipment: ${equipmentSummaryText({ available: workout.equipmentAvailable || [], preference: workout.equipmentPreference || 'mix' })}`;
...
async function updateTodayMetrics(defaultEquipment = []) {
  ...
  if ($('#equipmentSummary').textContent.includes('—')) {
    $('#equipmentSummary').textContent = `Equipment: ${equipmentSummaryText({ available: defaultEquipment, preference: 'mix' })}`;
  }
}
...
async function loadToday() {
  ...
  if (custom && custom.date === today) { appState.activeWorkoutSource = 'custom'; renderWorkoutChecklist(custom); $('#builderHint').textContent = 'Saved custom workout loaded for today.'; }
    ...
    if (plan) {
      ...
      if (session) { appState.activeWorkoutSource = 'plan'; renderWorkoutChecklist({ date: session.date, focus: session.focus, duration: (await idb.get('userProfile', 'profile')).sessionLength, exercises: session.exercises, source: 'plan', equipmentAvailable: sel.available, equipmentPreference: sel.preference }); }
    }
  }
  ...
  $('#completeWorkoutBtn').onclick = async () => {
    ...
    await safeSave('workoutLogs', { date: workout.date || formatDate(), focus: workout.focus, duration: Number(workout.duration) || 20, notes: '', difficulty: '', soreness: false, exercises: workout.exercises, source: appState.activeWorkoutSource, equipmentAvailable: workout.equipmentAvailable || [], equipmentPreference: workout.equipmentPreference || 'mix' });
